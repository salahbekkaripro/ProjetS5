...
class Command(BaseCommand):
    help = "Generate demo data: demo user, workouts over 30 days, orders, listings"
    ...
    def handle(self, *args, **options):
        ...
        Subscription.objects.filter(user=user, active=True).exclude(plan=fitplus).update(active=False)
        sub, _ = Subscription.objects.get_or_create(
            user=user,
            plan=fitplus,
            defaults={"active": True, "end_date": None},
        )
        if not sub.active:
            sub.active = True
            sub.end_date = None
            sub.save(update_fields=["active", "end_date"])
        ...
        p1, _ = Product.objects.get_or_create(
            name="T-Shirt Demo", defaults={"category": cat, "price": 19.99, "stock": 50}
        )
        p2, _ = Product.objects.get_or_create(
            name="Gourde Demo", defaults={"category": cat, "price": 12.99, "stock": 50}
        )
        ...
        order, _ = Order.objects.get_or_create(
            ...
            email=user.email,
            full_name="Demo User",
            defaults={"address": "1 rue Demo", "city": "Paris", "paid": True},
        )
        ...
        listing, _ = MarketplaceListing.objects.get_or_create(
            seller=user,
            title="Chaussures Demo",
            defaults={"description": "Comme neuves", "price": 49.0, "condition": "good"},
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Données demo générées. Utilisateur: demo@fittrackr.test / demo1234 (plan Fit+)"
            )
        )
